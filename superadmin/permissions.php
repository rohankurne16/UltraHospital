...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_permissions'])) {
    ...
    try {
        ...
        // Refresh role permissions for display
        $role_permissions = [];
        if ($selected_hospital > 0) {
...
if ($selected_role > 0 && $selected_hospital > 0) {
    $rp_query = "SELECT permission_id FROM role_permissions WHERE hospital_id = ? AND role_id = ?";
    $stmt = $conn->prepare($rp_query);
    $stmt->bind_param("ii", $selected_hospital, $selected_role);
    $stmt->execute();
    $rp_res = $stmt->get_result();
    while ($row = $rp_res->fetch_assoc()) {
        $role_permissions[] = $row['permission_id'];
    ...
    $rp_query = "SELECT DISTINCT permission_id FROM role_permissions WHERE role_id = ?";
    $stmt = $conn->prepare($rp_query);
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body.dark .card h3 { color: #f1f5f9 !important; }
        body.dark .form-label { color: #d1d5db; }
        body.dark .form-control { background: #1e1e1e; border-color: #2a2a2a; color: #f1f5f9; }
        body.dark .form-control:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,0.15); background: #2a2a2a; }
        body.dark .form-control::placeholder { color: #6b7280; }
        body.dark .icon-preview { background: #2a2a2a; color: #d1d5db; }
        body.dark .checkbox-group label { color: #d1d5db; }
        body.dark .checkbox-group input[type="checkbox"] { accent-color: #3b82f6; }
        body.dark .btn-secondary { background: #2a2a2a; color: #d1d5db; border-color: #3a3a3a; }
        body.dark .btn-secondary:hover { background: #3a3a3a; color: #f1f5f9; }
        body.dark .btn-secondary i { color: #9ca3af; }
        body.dark small { color: #6b7280 !important; }
        body.dark .required { color: #ef4444; }
        body.dark #addPermissionForm { border-top-color: #2a2a2a !important; }
        body.dark .tree-folder { background: #1a1a1a; border-color: #2a2a2a; }
        body.dark .tree-folder:hover { background: #262626; }
        body.dark .tree-file { background: #1a1a1a; }
        body.dark .tree-file:hover { background: #262626; }
        body.dark .tree-children { border-left-color: #3a3a3a; }
        ...
        .icon-preview i { font-size: 1.1rem; color: #3b82f6; }
        .checkbox-group {
            display: flex;
            gap: 1.5rem;
            align-items: center;
        ...
        @media (max-width: 480px) {
            ...
            .tree-children { margin-left: 0.8rem; }
        }
    </style>
</head>
<body>
    ...
    <div class="main-content" style="margin-top: 80px;">
       ...
       <!-- Add Permission Form – Improved Layout -->
<div class="card" style="padding: 1.25rem;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.5rem;">
        ...
        <button type="button" class="btn btn-secondary" onclick="toggleAddForm()" style="padding: 0.4rem 1rem; font-size: 0.8rem;">
    ...
    <div id="addPermissionForm" style="display: none; padding-top: 1rem; border-top: 1px solid #e2e8f0;">
        <form method="POST" onsubmit="return validatePermissionForm()">
            ...
            <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.2rem 1.5rem;">

                <!-- Permission Group -->
                <div>
                    <label class="form-label" style="display: block; font-size: 0.8rem; font-weight: 600; color: #475569; margin-bottom: 0.3rem;">
                        Permission Group <span class="required" style="color: #ef4444;">*</span>
                    </label>
                    <input type="text" name="permission_group" class="form-control" placeholder="e.g., Dashboard, Patients" required style="width: 100%; padding: 0.6rem 0.8rem; border-radius: 8px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 0.9rem; transition: all 0.2s; outline: none;">
                </div>

                <!-- Permission Name -->
                <div>
                    <label class="form-label" style="display: block; font-size: 0.8rem; font-weight: 600; color: #475569; margin-bottom: 0.3rem;">
                        Permission Name <span class="required" style="color: #ef4444;">*</span>
                    </label>
                    <input type="text" name="permission_name" class="form-control" placeholder="e.g., Dashboard View" required style="width: 100%; padding: 0.6rem 0.8rem; border-radius: 8px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 0.9rem; transition: all 0.2s; outline: none;">
                </div>

                <!-- Permission Slug -->
                <div>
                    <label class="form-label" style="display: block; font-size: 0.8rem; font-weight: 600; color: #475569; margin-bottom: 0.3rem;">
                        Permission Slug <span class="required" style="color: #ef4444;">*</span>
                    </label>
                    <input type="text" name="permission_slug" class="form-control" placeholder="e.g., dashboard-view" required style="width: 100%; padding: 0.6rem 0.8rem; border-radius: 8px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 0.9rem; transition: all 0.2s; outline: none;">
                    <small style="color: #94a3b8; font-size: 0.7rem; display: block; margin-top: 0.2rem;">Unique identifier (lowercase, hyphens only)</small>
                </div>
                ...
                <div>
                    ...
                    <select name="permission_icon" class="form-control" id="iconSelect" onchange="updateIconPreview()" style="width: 100%; padding: 0.6rem 0.8rem; border-radius: 8px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 0.9rem; outline: none;">
                    ...
                    <div style="margin-top: 0.5rem;">
                        <span class="icon-preview" style="display: inline-flex; align-items: center; gap: 0.6rem; padding: 0.3rem 0.9rem; background: #f1f5f9; border-radius: 6px; border: 1px solid #e2e8f0; font-size: 0.85rem; color: #475569;">
                            <i id="iconPreview" class="fas fa-circle" style="color: #3b82f6;"></i>
                            <span id="iconPreviewText" style="font-weight: 500;">fa-circle</span>
                        </span>
                    </div>
                </div>

                <!-- Visibility Settings (full width) -->
                <div class="full-width" style="grid-column: 1 / -1; margin-top: 0.2rem;">
                    <label class="form-label" style="display: block; font-size: 0.8rem; font-weight: 600; color: #475569; margin-bottom: 0.3rem;">
                        Visibility Settings
                    </label>
                    <div class="checkbox-group" style="display: flex; gap: 1.5rem; align-items: center; flex-wrap: wrap;">
                ...
                <!-- Buttons (full width) -->
                <div class="full-width" style="grid-column: 1 / -1; display: flex; gap: 0.75rem; padding-top: 0.8rem; border-top: 1px solid #e2e8f0; margin-top: 0.5rem;">
                    <button type="submit" name="add_permission" class="btn btn-success" style="padding: 0.5rem 1.5rem; border-radius: 8px; background: linear-gradient(135deg, #22c55e, #16a34a); color: #fff; border: none; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.2s;">
                        <i class="fas fa-save"></i> Add Permission
                    </button>
                    <button type="reset" class="btn btn-secondary" style="padding: 0.5rem 1.5rem; border-radius: 8px; background: #f1f5f9; color: #475569; border: 1px solid #e2e8f0; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem;">
        ...
        <!-- ============================================================ -->
        <!-- PERMISSIONS TREE VIEW (FIXED & IMPROVED) -->
        <!-- ============================================================ -->
    ...
    <span class="folder-icon"><i class="fas fa-folder"></i></span>  <!-- closed folder -->
    <span class="folder-name"><?php echo htmlspecialchars($group); ?></span>
    <span class="folder-badge"><?php echo count($perms); ?></span>
</div>
<div class="tree-children" id="<?php echo $group_id; ?>">   <!-- no 'open' class -->
                                <!-- Children -->
                                <div class="tree-children open" id="<?php echo $group_id; ?>">
                                    ...
                                    <?php foreach ($perms as $p): ?>
                                        <div class="tree-file">
                                            ...
                                            <label for="perm_<?php echo $p['permission_id']; ?>" style="display:flex; align-items:center; gap:0.4rem; cursor:pointer; width:100%; margin:0;">
    ...
    <script>
            ...
            folders.forEach(function(folder) {
                folder.addEventListener('click', function(e) {
                    ...
                    if (isOpen) {
                        ...
                        // Update folder icon
                        const icon = this.querySelector('.folder-icon i');
        ...
        // ============================================================
        // SELECT / DESELECT ALL (global and per group)
        // ============================================================
        function selectAll() {
            document.querySelectorAll('.tree-file input[type="checkbox"]').forEach(cb => cb.checked = true);
        }
        function deselectAll() {
            document.querySelectorAll('.tree-file input[type="checkbox"]').forEach(cb => cb.checked = false);
        }

        function selectGroup(groupId) {
            const container = document.getElementById(groupId);
            if (container) {
                container.querySelectorAll('.tree-file input[type="checkbox"]').forEach(cb => cb.checked = true);
            }
        }
        function deselectGroup(groupId) {
            const container = document.getElementById(groupId);
            if (container) {
                container.querySelectorAll('.tree-file input[type="checkbox"]').forEach(cb => cb.checked = false);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (hospitalSelect) {
                hospitalSelect.addEventListener('change', function() { this.form.submit(); });
            }
            if (roleSelect) {
                roleSelect.addEventListener('change', function() { this.form.submit(); });
            }
        });
